...
public class WikiModernFile implements WikiFile {
    ...
    private final static Logger LOG = Logger.getLogger(WikiModernFile.class);
    protected final static Pattern EXTRACT_STORE_AREA_REGEX = Pattern.compile("<div\\sid=\"storeArea\">(.*?)</div>\\s*<!--POST-STOREAREA-->", Pattern.DOTALL | Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE | Pattern.MULTILINE);
    private final String path;
    private final String content;
    private final WikiToHtmlConverter wikiToHtmlConverter;
    private final WikiClassicToPlainTextConverter wikiToPlainTextConverter;

    public WikiModernFile(String path, String content) {
        super();
        this.path = path;
        this.content = content;
        wikiToHtmlConverter = new WikiClassicToHtmlConverter();
        wikiToPlainTextConverter = new WikiClassicToPlainTextConverter();
    }

    private URI toURI(final String withBookmark) throws URISyntaxException {
    ...
    private List<Document> extractDocumentsFromStoreArea(final String storeArea) {
        ...
        final List<Document> elements = new ArrayList<>();
        ...
        while (tiddlerMatcher.find()) {
            ...
            if (wikiContent != null) {
                final String contentHtml = wikiToHtmlConverter.convertToHTML(wikiContent);
                doc.add(new Field("htmlContent", contentHtml, Field.Store.YES, Field.Index.NOT_ANALYZED));

                final String contentPlainText = wikiToPlainTextConverter.convertToPlainText(wikiContent);
    ...
    protected String extractStoreArea() {
        ...
        return extractStoreAreaMatcher.group(1).trim();
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (getClass() != obj.getClass())
            return false;
        WikiModernFile other = (WikiModernFile) obj;
        if (path == null) {
            if (other.path != null)
                return false;
